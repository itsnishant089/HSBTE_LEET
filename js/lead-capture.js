  ...
  function handleSubmit(event) {
    ...
    ensureSupabaseLoaded(function (loadError) {
      if (loadError) {
        ...
        // Fault tolerance: save lead status and bypass to avoid blocking the user
        try { localStorage.setItem('leetLeadCaptured', '1'); } catch (e) { }
        setMessage('Welcome to HSBTE LEET!', 'success');
      ...
      submitLead(form)
        ...
        .catch(function (err) {
          ...
          // Fault tolerance: even if save fails, let them browse the site
          try { localStorage.setItem('leetLeadCaptured', '1'); } catch (e) { }
          setMessage('Welcome to HSBTE LEET!', 'success');
          setTimeout(hideModal, 1000);
        })
