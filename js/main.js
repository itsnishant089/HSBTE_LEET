...
// ================================
// ADSense CONTAINER FIX - Hide Unfilled Ads
// ================================
function hideUnfilledAdSense() {
  // Hide unfilled AdSense containers
  ...
  // Hide any AdSense containers that appear after footer
  const footer = document.querySelector('footer');
  if (footer) {
    const elementsAfterFooter = Array.from(document.body.children).filter(
      el => el.tagName !== 'SCRIPT' && el.tagName !== 'STYLE' && 
      footer.compareDocumentPosition(el) & Node.DOCUMENT_POSITION_FOLLOWING
    );
    
    elementsAfterFooter.forEach(el => {
      if (el.matches && (el.matches('ins.adsbygoogle') || el.matches('iframe[src*="googlesyndication"]') || el.matches('div[id*="google_ads"]'))) {
...
// Run on DOM ready
document.addEventListener('DOMContentLoaded', hideUnfilledAdSense);

// Run after partials load
document.addEventListener('partialsLoaded', () => {
  setTimeout(hideUnfilledAdSense, 500);
});

// Monitor for AdSense status changes
...
// Also run periodically to catch dynamically loaded ads
setInterval(hideUnfilledAdSense, 2000);
